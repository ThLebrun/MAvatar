...
class InferenceEvaluator:
    ...
    def evaluate(self, n_jobs: int = -2) -> "InferenceEvaluator":
        ...
        """
        self._n_baseline, self.result_baseline = self._attack(target=self._ori, naive=True, n_jobs=n_jobs)
        self._n_success, self.result_success = self._attack(target=self._ori, naive=False, n_jobs=n_jobs)
        self._n_control, self.result_control = (
            (None, None) if self._control is None else self._attack(target=self._control, naive=False, n_jobs=n_jobs)
        )
